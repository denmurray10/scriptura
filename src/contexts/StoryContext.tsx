...
import { analyzeChoiceAction, discoverAndAddSceneAction, recruitNewCharacterAction, generateStoryCoverAction, type GenerateCharacterDetailsOutput, type GenerateCharacterImagesOutput, generateChapterSummaryAction, generateNarratorTurnAction, generateFullStoryAction, regenerateCoverAction, regenerateCharacterImagesAction, regenerateSceneVisualAction, generateStoryMapAction, regenerateStoryMapAction, generateScenarioVisualAction, generateAudioTeaserAction, generateSpeechAction } from "@/lib/actions";
import type { Character, Scene, Story, AppState, Item, Scenario, StoryHistoryEntry, Objective, ShopItem, TokenPackage, Relationship, Shelf, ReadingGoal, RelationshipEvent, Player, DashboardConfig, MiniGame } from "@/types";
...
interface StoryContextType {
  ...
  setStoryMode: (storyId: string, mode: 'immersive' | 'narrator' | 'audio') => Promise<void>;
  ...
  handleHagglingOffer: (offer: number) => Promise<void>;
}
...
const INITIAL_STATE: AppState = {
  ...
  aboutMe: undefined,
  dashboardConfig: {
    featuredBannerStoryIds: [],
    bookOfTheDayStoryId: undefined,
  },
};
...
export const StoryProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const coOpQuery = query(collection(db, "stories"), where("playerIds", "array-contains", currentUser.uid));
    unsubscribes.push(onSnapshot(coOpQuery, (snapshot) => {
        const playerStories = processSnapshot(snapshot);
  ...
  useEffect(() => {
    ...
    const appStateDocRef = doc(db, "users", currentUser.uid, "appState", "main");
    unsubscribes.push(onSnapshot(appStateDocRef, (docSnap) => {
    ...
    // Listener for dashboard config
    const dashboardConfigDocRef = doc(db, "admin", "dashboardConfig");
    unsubscribes.push(onSnapshot(dashboardConfigDocRef, (docSnap) => {
        if (docSnap.exists()) {
            setState(prevState => ({
                ...prevState,
                dashboardConfig: {
  ...
  const updateStory = async (storyId: string, updatedData: Partial<Omit<Story, "id">>) => {
    ...
    setState(prevState => {
        const stories = prevState.stories.map(s =>
            s.id === storyId ? { ...s, ...updatedData } as Story : s
        );
        return { ...prevState, stories };
    });
  
    // Find the story *after* the state update to get the most current version
    ...
    try {
        ...
        console.error("Failed to update story in DB. Error:", e);
        toast({
            variant: "destructive",
            title: "Sync Error",
            description: "Could not save story changes to the cloud.",
        });
    }
  };
  ...
  const createStory = async (storyData: Omit<Story, 'id' | 'characters' | 'scenes' | 'storyStatus' | 'storyHistory' | 'activeCharacterId' | 'locationName' | 'timeOfDay' | 'currentSceneIndex' | 'coverType' | 'imageUrl' | 'videoUrl' | 'coverSize' | 'objectives' | 'interactionsUntilNextObjective' | 'storyMode' | 'isBookmarked' | 'lastPlayedTimestamp' | 'createdAt' | 'shelfId' | 'audioTeaserUrl' | 'authorId' | 'authorName' | 'isManuallyCreated' | 'authorPhotoURL' | 'authorAboutMe' | 'teaser' | 'mapUrl' | 'lastLocationName' | 'lastRelationshipEvent' | 'language' | 'inviteCode' | 'isCoop' | 'players' | 'playerIds' | 'turnCharacterId' | 'activeMiniGame' | 'temporaryNpcs'>, scenes: Scene[], characters: Character[]): Promise<Story | undefined> => {
    ...
    try {
        const { imageUrl, videoUrl, ...restOfStoryData } = storyData;

        const [finalCoverUrl, finalVideoUrl, audioResult] = await Promise.all([
          imageUrl ? uploadDataUrlToFirebase(imageUrl, 'covers') : Promise.resolve(''),
          videoUrl ? uploadDataUrlToFirebase(videoUrl, 'covers') : Promise.resolve(''),
          generateSpeechAction({ text: storyData.plot }).catch(() => null)
        ]);
        ...
        const newStoryForDb: Omit<Story, 'id'> = {
            ...
            coverType: finalVideoUrl ? 'video' : 'image',
            coverSize: imageUrl ? getBase64Size(imageUrl) : 0,
            mapUrl: '', // Will be generated later
            ...
            ...(storyData.isCoop && { inviteCode: crypto.randomUUID().slice(0, 6).toUpperCase() }),
            temporaryNpcs: [],
            activeMiniGame: null,
        };
  ...
  const setStoryMode = async (storyId: string, mode: 'immersive' | 'narrator' | 'audio') => {
    if (!currentUser) return;
  ...
  const startStory = async () => {
    ...
    updateData = {
      ...
      activeCharacterId: activeStory.activeCharacterId || activeStory.characters.find(c => c.isPlayable)?.id || null,
    };
  ...
  const makeChoice = async (choice: string) => {
    ...
    try {
      ...
      if (activeStory.storyMode === 'narrator') {
          ...
          const newScenario: Scenario = { description: effects.newScenario };
          const updatedCharacters = activeStory.characters.map(c => ({...c, currentScenario: newScenario}));
          ...
          const updatedCharactersMap = new Map(activeStory.characters.map(c => [c.id, { ...c, currentScenario: newScenario }]));
  ...
  const regenerateCharacterImages = async (storyId: string, characterId: string) => {
    ...
    const updatedCharacters = story.characters.map(c => c.id === characterId ? updatedCharacter : c);
  ...
  const regenerateSceneVisual = async (storyId: string, sceneId: string) => {
    ...
    const updatedScenes = story.scenes.map(s => s.id === sceneId ? updatedScene : s);
    
    await updateStory(storyId, { scenes: updatedScenes });
  };
  ...
  const regenerateStoryMap = async (storyId: string) => {
    ...
    await updateStory(storyId, { mapUrl: updatedUrl });
  };
  ...
  const updateDashboardConfig = async (config: Partial<DashboardConfig>) => {
    const configDocRef = doc(db, "admin", "dashboardConfig");
  ...
  const handleHagglingOffer = async (offer: number) => {
    if (!activeStory || !activeStory.activeMiniGame || activeStory.activeMiniGame.type !== 'haggling') return;

    const gameData = activeStory.activeMiniGame.data;
    try {
        const result = await handleHagglingTurnAction({
            npcName: gameData.npcName,
            item: gameData.item,
            targetPrice: gameData.targetPrice,
            patience: gameData.patience,
            playerOffer: offer,
        });

        const newPatience = gameData.patience - result.patienceDamage;
        const newStatus = result.isDeal ? 'won' : newPatience <= 0 ? 'lost' : 'active';
        
        const finalDialogue = newStatus === 'lost' ? "I've had enough of this. The deal is off!" : result.dialogue;

        const updatedGame: MiniGame = {
            ...activeStory.activeMiniGame,
            status: newStatus,
            data: {
                ...gameData,
        ...
        if (newStatus === 'won' || newStatus === 'lost') {
            await updateStory(activeStory.id, { activeMiniGame: null, temporaryNpcs: [] });
            // Potentially add a history entry here about the outcome
        } else {
            await updateStory(activeStory.id, { activeMiniGame: updatedGame });
        }

    } catch (e) {
        console.error("Haggling turn failed:", e);
        toast({ variant: 'destructive', title: 'Haggling Error', description: 'Could not process the offer.' });
    }
  };
  ...
  const value: StoryContextType = {
    ...
    dashboardConfig: state.dashboardConfig || { featuredBannerStoryIds: [], bookOfTheDayStoryId: undefined },
